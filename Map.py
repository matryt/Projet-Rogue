...
class Map(object):
	...
	def moveAllMonsters(self):
		...
		for m in self._elem:
			...
			if (
				...
				d = posMonster.direction(posHero)
	...
	def findPath(self, liste_ouverte, liste_fermee, courant, arrivee):
		n_depart = Noeud.Noeud(position=courant)
		n_depart.cout_g = 0
		n_depart.cout_h = courant.distance(arrivee)
		n_depart.cout_f = n_depart.cout_g + n_depart.cout_h
		liste_ouverte[courant] = n_depart
		for d in Map.dir.values():
			...
			cout_g = liste_ouverte[courant].cout_g + courant.distance(newCoord)

			# Si la case n'est pas dans la liste ouverte, on l'y ajoute
			if newCoord not in liste_ouverte:
				n = Noeud.Noeud(position=newCoord)  # initialisation de n avec la position
				n.parent = liste_ouverte[courant]
				n.cout_g = cout_g
				n.cout_h = newCoord.distance(arrivee)
				...
				if cout_g < tmp.cout_g:
					tmp.parent = liste_ouverte[courant]
					tmp.cout_g = cout_g
					tmp.cout_f = cout_g + tmp.cout_h

	def ajouter_cases_adjacentes(self, courant, arrivee: Coord, liste_fermee: dict, liste_ouverte: dict):
		n = Noeud.Noeud()
		n.parent = courant
		...
		for d in Map.dir.values():
			...
			# Calcul du coût G de la case
			cout_g = n.cout_g + courant.distance(newCoord)
