...
class Hero(Creature.Creature):
    ...
    def opendescription(self, item, floor):
        choice = getch()
        try:
            c = int(choice)
            if c < 0 or c > 3:
            ...
            self.opendescription(item, floor)
        ...
        if c == 3:
            self._inventory.remove(item)
        if c == 2:
            voisins = floor.pos(self).voisins(floor)
            for v in voisins:
                if floor.get(v) != Map.Map.empty:
                    voisins.remove(v)
            if voisins:
                floor.put(random.choice(voisins), item)
                self._inventory.remove(item)
            else:
