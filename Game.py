...
class Game(object):
    ...
    equipments = {
        0: [
            ...
            Wearable.Wearable(
                "broken sword", place="right hand", effect={"strength": 1}, usage=lambda self, hero: equip(hero, self)
            ),
            Wearable.Wearable(
                "trident", place="right hand", effect={"strength": 3}, usage=lambda self, hero: equip(hero, self)
            ),
            Wearable.Wearable(
    ...
    _actions = {
        ...
        " ": lambda hero: None,
        "u": lambda hero: hero.opendescription(
            theGame.theGame().select(hero.getInventory()), theGame.theGame().getFloor()
        ),
    ...
    def buildFloor(self, s=False):
        ...
        if nbRooms >= 2 and 5 <= self._level <= 15:
            self._floor.put(
